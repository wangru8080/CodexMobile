...
状态：部分完成（3.4a 普通语音输入已完成，3.4b 已抽取运行状态与完成后消息刷新，聊天发送/轮询仍在 App）
...
- Commit：fcc6e39、6563b7e；当前 3.4b 刷新子步未提交
- 验证命令：`npm run build` 通过
- 备注：3.4a 新增 `useVoiceInput`，迁移普通语音提交入口；3.4b 新增 `useChatTurns`，已迁移运行状态、运行 ref、轮询集合、刷新定时器清理、完成后消息刷新和完成状态更新。
