...
- 第一阶段：常量与工具函数拆分，已完成。
- 第二阶段：UI 组件拆分，已完成。
- 第三阶段：hooks 拆分，进行中；第 3.1、3.2、3.3 批已完成，第 3.4a 已完成，第 3.4b 部分完成。
...
状态：部分完成（3.4a 普通语音输入已完成，3.4b 聊天运行状态已抽取，聊天发送/轮询仍在 App）
...
- Commit：fcc6e39；当前 3.4b 子步未提交
- 验证命令：`npm run build` 通过
- 备注：3.4a 新增 `useVoiceInput`，迁移普通语音提交入口；3.4b 新增 `useChatTurns`，已迁移运行状态、运行 ref、轮询集合和刷新定时器清理。
