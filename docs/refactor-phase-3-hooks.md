...
状态：部分完成（3.4a 普通语音输入已完成，3.4b 已抽取运行状态与完成后消息刷新，聊天发送仍在 App，中止任务已抽取）
...
- Commit：fcc6e39、6563b7e、8307a96、3298f67；当前中止子步未提交
- 验证命令：`npm run build` 通过
- 备注：3.4a 新增 `useVoiceInput`，迁移普通语音提交入口；3.4b 新增 `useChatTurns`，已迁移运行状态、运行 ref、轮询集合、刷新定时器清理、完成后消息刷新、完成状态更新、turn 会话绑定、turn 消息加载、轮询和中止任务。
